...
//import { useAuth } from '../../Providers/useAuth'; // safer hook

const Profile = () => {
  const { user, loading } = useAuth(); // Use custom hook to safely get context
  const [isEditing, setIsEditing] = useState(false);
  const [avatarPreview, setAvatarPreview] = useState('/api/placeholder/200/200'); // default
  const [formData, setFormData] = useState({
    name: '',
    email: '',
    phone: '',
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-purple-600 text-xl">Loading Profile...</p>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-red-600 text-xl">You must be logged in to view this page.</p>
      </div>
    );
  }
  ...
  if (!formData.name) {
    setFormData({
      ...
      phone: user.phone || '+1 (555) 123-4567',
      location: user.location || 'San Francisco, CA',
      bio: user.bio || 'Passionate curator and admin of the Artify digital art marketplace.',
    });
    setAvatarPreview(user.photoURL || '/api/placeholder/200/200');
  }
  ...
  const handleSave = () => {
    ...
    // Real implementation: update context + backend
  };
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">Role</p>
                  <p className="font-bold text-emerald-600">{formData.role}</p>
                </div>
              </div>
            </div>
              ...
              {isEditing ? (
                <>
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className={`px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl font-bold hover:shadow-xl transition ${saving ? 'opacity-70 cursor-not-allowed' : 'hover:scale-105'}`}
                  >
